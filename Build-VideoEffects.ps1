...
function Build-Platform {
    ...
    Write-Host
    Write-Host "Building FFmpegInteropX.VideoEffects for Windows 10 ${Platform}..."
    Write-Host
    ...
    if ($ClearBuildFolders) {
        ...
        Remove-Item -Force -Recurse -ErrorAction Ignore $SolutionDir\Intermediate\FFmpegInteropX.VideoEffects\$Platform\${Configuration}_${WindowsTarget}
        Remove-Item -Force -Recurse -ErrorAction Ignore $SolutionDir\Output\FFmpegInteropX.VideoEffects\$Platform\${Configuration}_${WindowsTarget}
    }

    MSBuild.exe $SolutionDir\VideoEffects\FFmpegInteropX.VideoEffects.vcxproj `
        /restore `
    ...
    if ($lastexitcode -ne 0) { throw "Failed to build library FFmpegInteropX.VideoEffects.vcxproj." }

}

Write-Host
Write-Host "Building FFmpegInteropX.VideoEffects..."
Write-Host
